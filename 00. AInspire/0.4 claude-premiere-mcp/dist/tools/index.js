...
export class PremiereProTools {
        ...
        try {
            switch (name) {
                ...
                case 'assemble_product_spot':
                    return await this.assembleProductSpot(args);
                case 'build_brand_spot_from_mogrt_and_assets':
                    return await this.buildBrandSpotFromMogrtAndAssets(args);
                // Project Management
                case 'create_project':
    ...
    async listProjectItems(includeBins = true, _includeMetadata = false) {
        const script = `
      try {
    ...
    async buildMotionGraphicsDemo(sequenceName = 'Apple Like Motion Demo') {
        ...
        const clips = placements.map((placement) => placement.id).filter(Boolean);
        const transitions = [];
        if (clips[0]) {
    ...
    hasColorAdjustments(color) {
        if (!color) {
            return false;
        }
        return Object.values(color).some((value) => value !== undefined);
    }
    async assembleProductSpot(args) {
        ...
        const motionStyle = args.motionStyle ?? (hasDirectedPlan ? 'none' : 'alternate');
        ...
        const planSteps = hasDirectedPlan
            ? args.clipPlan ?? []
        ...
        for (let index = 0; index < planSteps.length; index++) {
            ...
            const placement = await this.addToTimeline(createdSequence.id, importedAsset.id, track, placementTime, insertMode);
            placements.push(placement);
        ...
        for (let index = 0; index < placements.length - 1; index++) {
            ...
            if (transitionToApply) {
                transitions.push(await this.addTransitionToClip(placements[index].id, transitionToApply, 'end', durationToApply));
            }
        }
        const animations = [];
        for (let index = 0; index < placements.length; index++) {
            ...
            const style = motion?.style ?? motionStyle;
    ...
    async buildBrandSpotFromMogrtAndAssets(args) {
        const assemblyArgs = {
            sequenceName: args.sequenceName,
        ...
        const overlays = [];
        if (args.mogrtPath) {
            overlays.push(await this.importMogrt(assembly.sequence.id, args.mogrtPath, args.titleStartTime ?? 0.4, args.titleTrackIndex ?? 1, 0));
        }
        else {
            overlays.push({
                success: true,
    ...
    async moveClip(clipId, newTime, _newTrackIndex) {
        const script = `
      try {
        var info = __findClip("${clipId}");
        if (!info) return JSON.stringify({ success: false, error: "Clip not found" });
    ...
    async razorTimelineAtTime(sequenceId, time, videoTrackIndices, audioTrackIndices) {
    ...
    async applyEffect(clipId, effectName, _parameters) {
        const script = `
      try {
        app.enableQE();
        var info = __findClip("${clipId}");
    ...
    async addTransition(clipId1, _clipId2, transitionName, duration) {
        const script = `
      try {
        app.enableQE();
    ...
    async applyLut(clipId, lutPath, _intensity = 100) {
        const script = `
      try {
        app.enableQE();
        var info = __findClip("${clipId}");
    ...
    async stabilizeClip(clipId, _method = 'warp', smoothness = 50) {
        const script = `
      try {
        app.enableQE();
        var info = __findClip("${clipId}");
    ...
    async replaceClip(clipId, newProjectItemId, _preserveEffects) {
        const script = `
      try {
        var info = __findClip(${JSON.stringify(clipId)});
        if (!info) return JSON.stringify({ success: false, error: "Clip not found" });
    ...
    async setClipProperties(clipId, properties) {
        const propCode = [
    ...
    async getPlayheadPosition(sequenceId) {
        const script = `
      try {
    ...
    async setPlayheadPosition(sequenceId, time) {
        const script = `
      try {
        var sequence = __findSequence(${JSON.stringify(sequenceId)});
        if (!sequence) sequence = app.project.activeSequence;
    ...
    async getSelectedClips(sequenceId) {
        const script = `
      try {
        var sequence = __findSequence(${JSON.stringify(sequenceId)});
        if (!sequence) sequence = app.project.activeSequence;
    ...
    async listAvailableAudioEffects() {
        const script = `
      try {
        app.enableQE();
    ...
    async listAvailableAudioTransitions() {
        const script = `
      try {
        app.enableQE();
    ...
    async addKeyframe(clipId, componentName, paramName, time, value) {
        const script = `
      try {
        var info = __findClip(${JSON.stringify(clipId)});
        if (!info) return JSON.stringify({ success: false, error: "Clip not found" });
    ...
    async removeKeyframe(clipId, componentName, paramName, time) {
        const script = `
      try {
        var info = __findClip(${JSON.stringify(clipId)});
        if (!info) return JSON.stringify({ success: false, error: "Clip not found" });
    ...
    async getKeyframes(clipId, componentName, paramName) {
        const script = `
      try {
        var info = __findClip(${JSON.stringify(clipId)});
        if (!info) return JSON.stringify({ success: false, error: "Clip not found" });
    ...
    async setWorkArea(sequenceId, inPoint, outPoint) {
        const script = `
      try {
        var sequence = __findSequence(${JSON.stringify(sequenceId)});
        if (!sequence) sequence = app.project.activeSequence;
    ...
    async batchAddTransitions(sequenceId, trackIndex, transitionName, duration) {
        const script = `
      try {
        app.enableQE();
        var sequence = __findSequence(${JSON.stringify(sequenceId)});
    ...
    async findProjectItemByName(name, type) {
    ...
    async moveItemToBin(projectItemId, targetBinId) {
        const script = `
      try {
    ...
    async setActiveSequence(sequenceId) {
        const script = `
      try {
    ...
    async getClipAtPosition(sequenceId, trackType, trackIndex, time) {
        const script = `
      try {
        var sequence = __findSequence(${JSON.stringify(sequenceId)});
        if (!sequence) sequence = app.project.activeSequence;
    ...
    async autoReframeSequence(sequenceId, numerator, denominator, motionPreset, newName) {
    ...
    async detectSceneEdits(sequenceId, action, applyCutsToLinkedAudio, sensitivity) {
    ...
    async createCaptionTrack(sequenceId, projectItemId, startTime, captionFormat) {
    ...
    async createSubclip(projectItemId, name, startTime, endTime, hasHardBoundaries, takeAudio, takeVideo) {
    ...
    async relinkMedia(projectItemId, newFilePath) {
        const script = `
      try {
        var item = __findProjectItem(${JSON.stringify(projectItemId)});
        if (!item) return JSON.stringify({ success: false, error: "Project item not found" });
    ...
    async setColorLabel(projectItemId, colorIndex) {
        const script = `
      try {
        var item = __findProjectItem(${JSON.stringify(projectItemId)});
        if (!item) return JSON.stringify({ success: false, error: "Project item not found" });
    ...
    async getColorLabel(projectItemId) {
        const script = `
      try {
        var item = __findProjectItem(${JSON.stringify(projectItemId)});
        if (!item) return JSON.stringify({ success: false, error: "Project item not found" });
    ...
    async getMetadata(projectItemId) {
        const script = `
      try {
        var item = __findProjectItem(${JSON.stringify(projectItemId)});
        if (!item) return JSON.stringify({ success: false, error: "Project item not found" });
    ...
    async setMetadata(projectItemId, key, value) {
        const script = `
      try {
        var item = __findProjectItem(${JSON.stringify(projectItemId)});
        if (!item) return JSON.stringify({ success: false, error: "Project item not found" });
    ...
    async getFootageInterpretation(projectItemId) {
        const script = `
      try {
        var item = __findProjectItem(${JSON.stringify(projectItemId)});
        if (!item) return JSON.stringify({ success: false, error: "Project item not found" });
    ...
    async setFootageInterpretation(projectItemId, frameRate, pixelAspectRatio) {
    ...
    async exportAsFcpXml(sequenceId, outputPath) {
        const script = `
      try {
        var seq = __findSequence(${JSON.stringify(sequenceId)});
        if (!seq) return JSON.stringify({ success: false, error: "Sequence not found" });
    ...
    async setSequenceInOutPoints(sequenceId, inPoint, outPoint) {
    ...
    async getSequenceInOutPoints(sequenceId) {
        const script = `
      try {
        var seq = __findSequence(${JSON.stringify(sequenceId)});
        if (!seq) return JSON.stringify({ success: false, error: "Sequence not found" });
    ...
    async exportAaf(sequenceId, outputPath, mixDownVideo, explodeToMono, sampleRate, bitsPerSample) {
    ...
    async refreshMedia(projectItemId) {
        const script = `
      try {
        var item = __findProjectItem(${JSON.stringify(projectItemId)});
        if (!item) return JSON.stringify({ success: false, error: "Project item not found" });
    ...
    async importSequencesFromProject(projectPath, sequenceIds) {
        const script = `
      try {
    ...
    async createSubsequence(sequenceId, ignoreTrackTargeting) {
    ...
    async importMogrt(sequenceId, mogrtPath, time, videoTrackIndex, audioTrackIndex) {
    ...
    async importMogrtFromLibrary(sequenceId, libraryName, mogrtName, time, videoTrackIndex, audioTrackIndex) {
        const vidTrack = videoTrackIndex || 0;
        const audTrack = audioTrackIndex || 0;
        const script = `
      try {
    ...
    async manageProxies(projectItemId, action, proxyPath) {
        const script = `
      try {
        var item = __findProjectItem(${JSON.stringify(projectItemId)});
        if (!item) return JSON.stringify({ success: false, error: "Project item not found" });
