...
4. 🔌 **생성 엔진 연결** (전부 OAuth, API 키 불필요 — 셋 중 최소 하나면 됨):
   - **Higgsfield CLI** *(기본 — 이것만으로 전 과정 가능)* — ① `npm install -g @higgsfield/cli` ② `higgsfield auth login`(브라우저 5초) ③ (선택) `npx skills add higgsfield-ai/skills`(에이전트 연동 → "Higgsfield로 이미지 만들어줘"). 확인 `higgsfield account status`.
   - **Higgsfield MCP** *(백업 — CLI 없을 때)* — 데스크톱 앱 로그인. 확인 `balance`.
...
| 스토리보드 HTML(base64 임베드) | `_build_VELVET_NOIR_embed.py` *(VELVET NOIR 템플릿)* |
| 스토리보드 HTML(base64 임베드) | `_build_TETHER_embed.py` 등 *(TETHER/VOLT/ENERGYRUNNER 템플릿)* |
| 최종 완성 스토리보드 HTML | `_build_final_storyboards.py` *(HALO/AETHER 예시 템플릿)* |
| 인터랙티브 리뷰 서버(Higgsfield CLI 직접 호출) | `_review_server.py` — `python3 _review_server.py --project VELVET_NOIR --version v20260626 --port 7800` |
| 내레이션 TTS(ElevenLabs 직접 API, 백업2 경로) | `_gen_halo_vo.py` *(템플릿 — 기본은 ElevenLabs MCP 사용, 이 스크립트는 MCP 연결이 없을 때 직접 API 백업으로만)* |
| 로고 루마키 투명화 | `_logo_transparent.py` · `_logo_transparent_TETHER.py` · `_logo_transparent_ENERGYRUNNER.py` *(템플릿)* |
...
## ⚠️ *(템플릿)* 파일 주의
`_build_halo_v5_embed.py` · `_build_VELVET_NOIR_embed.py` · `_build_TETHER_embed.py` 등 `_build_*` 파일,
`_build_final_storyboards.py` · `_gen_halo_vo.py` · `_logo_transparent*.py`는 각 프로젝트 광고 예시가 든 **작업 템플릿**이다.
내부의 `projects/halo_watch/...` 경로 · `JOBS` · `voice_id` · 파일명은 **예시값**이라
새 프로젝트에선 그 파일이 아직 없다 → 자기 프로젝트 값으로 교체해야 동작한다. **`ROOT`만은 자동 탐지라 손댈 필요 없다.**
`_review_server.py`는 예외 — 프로젝트명/버전을 인자로 받는 범용 서버이므로 내부 경로를 수정하지 않아도 된다.
