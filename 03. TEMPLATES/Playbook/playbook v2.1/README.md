# Portable Storyboard Kit v2 — 광고 스토리보드 제작 묶음 (Higgsfield CLI/MCP · Magnific)

광고 영상 한 편을 **기획 → 스토리보드 → 영상화 → 내레이션 → 로고 → 최종 조립**까지 만드는 휴대용 묶음.
다른 컴퓨터에서도 그대로 동작하도록 **모든 경로는 상대경로**이고, 스크립트의 `ROOT`는 파일 위치에서 자동 탐지된다.

> 이미지·영상 생성은 **Higgsfield(CLI 또는 MCP) / Magnific MCP**로,
> 내레이션은 **Higgsfield TTS(`inworld_text_to_speech`)**로 한다(별도 키 불필요, 대안: ElevenLabs 직접 API). BGM은 **Suno 프롬프트를 제안받아 사용자가 직접 생성**(GATE 13, 키 불필요).
...
playbook v2.1/                                ← 이 폴더째 공유. 여기서 에이전트를 연다.
  ...
  system_v2/                                   ← 후처리·유틸 스크립트 + .env (ElevenLabs 키만 채우면 됨)
  review_console/                              ← GATE 17 리뷰 콘솔(선택) — start.py 원클릭
```

## 새 컴퓨터 준비
1. **Node.js 18+** — `.mjs`용(전역 `fetch` 사용, 외부 npm 0개).
2. **Python 3.9+ + Pillow** (`pip install Pillow`) — 스토리보드 HTML 빌드·로고 투명화.
3. **ffmpeg** — 영상 조립(`assemble_video.mjs`)용.
...
5. **내레이션** — Higgsfield 연결(위 4번)만 있으면 OK, 별도 키·`.env` 불필요. CLI `higgsfield generate create inworld_text_to_speech --prompt "..."` 또는 MCP `generate_audio`로 생성.
   *(대안: ElevenLabs 직접 API 쓰려면 `system_v2/.env`를 열어 `ELEVENLABS_API_KEY` 입력(파일은 이미 있음), `node check_api_connections.mjs`로 확인.)*
   ```
   # system_v2 폴더에서 (ElevenLabs 쓸 때만)
...
| 내레이션 TTS(ElevenLabs, 대안 경로) | `_gen_halo_vo.py` *(템플릿 — 기본은 Higgsfield TTS 사용, 이 스크립트는 ElevenLabs 쓸 때만)* |
...
> **이미지·영상 생성은 system_v2 스크립트가 아니라 생성 엔진**(Higgsfield CLI/MCP · Magnific)으로 한다 — `00_core.md`의 [생성 엔진] 참조.
...
## 보안
- `.env`는 **빈 채로 포함**(키만 채우면 됨). **배포본의 `.env`엔 절대 실제 키를 넣지 말 것** — 키는 받는 사람이 입력. `.gitignore`가 `.env`·로그·`__pycache__` 커밋 차단.
