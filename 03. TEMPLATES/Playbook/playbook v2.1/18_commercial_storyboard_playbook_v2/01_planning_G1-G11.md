...
제안한 서사가 계속 안 통하면, **그 카테고리의 실제 명작 광고를 웹에서 분석**해 검증된 문법을 빌려온다. 스마트워치(애플워치) 기준 아키타입:
| 문법 | 대표작 | 특징 | 어울리는 톤 |
|---|---|---|---|
| **리얼 스토리(생명·안심)** | Apple Watch 〈911〉 | 실제 위기→기기가 구함→"덕분에 무사" 자막 | 긴장→안도, 임팩트 |
| 키네틱 피트니스 몽타주 | 〈Close Your Rings〉/〈Go Time〉 | 고템포·빠른 컷·활동링, 음악 비트 싱크 | 에너제틱 |
| 테스티모니얼 모자이크 | 〈Dear Apple〉 | 실사용자 한마디 자막 + 인물 컷 모음 | 진정성, 캠페인 확장 |
| 싱글 라이프스타일 비네트 | 일상편 | 한 사람의 하루에 제품이 녹음 | 잔잔·프리미엄 |
> **시행착오(HALO 최종):** 감정 서사(거리·회복·자기돌봄)를 다 "별로"라고 한 뒤, 애플워치 광고를 **실제로 웹 검색·분석**해서 4개 아키타입을 제안 → 사용자가 **〈911〉 리얼 스토리**를 선택. **막히면 레퍼런스를 분석하라.**
...
### G9a-rev — 개별 컷 수정 (nano_banana Pro · 16:9 · 2K · 1/4 output · Unlimited ON)
...
## GATE 10 — 스토리보드 HTML 빌드 & 검수

### HTML 조립 (`system_v2/_build_{name}_embed.py`)
- 각 png를 **Pillow로 width 760 리사이즈 → JPEG q80 → base64**로 박아 **자체완결 HTML 1파일**을 만든다(외부 이미지 의존 없음).
- 컷마다 카드: 썸네일 + 번호 + 막 태그 + 한글 장면 + 제품 역할.
- 출력 파일은 **반드시 그 버전 폴더 안에** 저장. (`storyboard_{NAME}_30cut_v{날짜}_vN.html`)

### Q10-1. 검수 방식 (객관식)
- A. 사용자가 브라우저로 직접 열어 확인 [기본 — 규칙 3]
- B. 추가로 자동 검수 필요(별도 품질 검수 루틴)

> **시행착오 #10:** 생성 이미지를 에이전트가 Read로 열지 마라(컨텍스트 폭증 + 20MB 제한). 경로만 안내하고 사람이 본다.
> **→ 검수 자동화:** GATE 17의 리뷰 콘솔을 자동 팝업하면, 사용자가 카드에서 바로 컷을 골라 수정요청을 Enter로 보내고 즉시 재생성할 수 있다.
