...
| **영상화** | **Higgsfield CLI / MCP(`seedance_2_0` "Seedance 2.0 Mini", 기본) · Magnific MCP** | CLI `higgsfield generate create seedance_2_0 … --image --duration 4 --resolution 720p --generate_audio false` · MCP `generate_video` · Magnific `video_generate` (Mini 미확인/실패 시 백업1: `--mode fast`("Seedance 2.0 Fast") → 그래도 안 되면 백업2·최종: `--mode std`("Seedance 2.0 standard")) |
...
4. 🤖 **기본 모델(2026-06-22부터):** Higgsfield 연결 시 **개별 컷 이미지 = `nano_banana_2`(Pro) · 2K · Unlimited ON**, **영상 = `seedance_2_0`("Seedance 2.0 Mini" 프리셋) · 720p · 4초**를 기본값으로 쓴다. `seedance_2_0`는 `medias`에 `image`/`start_image`/`end_image`/`video`/`audio` role을 받고, `resolution`은 `480p`/`720p`/`1080p` 중 선택(기본·이 플레이북 모두 720p), `duration`은 4~15초 범위(기본 5, 이 플레이북은 4로 고정)이며 `generate_audio`가 **기본 true**라 규칙0[B](No-BGM)를 지키려면 호출마다 반드시 `generate_audio:false`로 명시한다(`higgsfield model get seedance_2_0 --json` 기준, 2026-06-22 확인). ⚠️ "Seedance 2.0 Mini"는 2026-06-22 Higgsfield 웹 UI에 신규(EXCLUSIVE) 노출된 모델로, 이 시점 CLI/MCP `models_explore` 카탈로그에는 별도 model_id로 아직 안 잡힌다(`seedance_2_0` 하나만 조회됨) — 생성 직전 `models_explore(action='search', query='seedance 2.0 mini')` 또는 CLI `higgsfield model list --video`로 별도 model_id 등장 여부를 재확인한다.
   **3단 백업 체인(전부 `seedance_2_0`, `mode` 파라미터만 다름 — Kling 3.0 Turbo는 백업에서 제외):** ① **Mini**(별도 model_id가 카탈로그에서 확인된 경우에만 그 model_id로 720p·4초·`generate_audio:false` 호출) → ② **Mini가 카탈로그에 안 잡히면 바로 백업1** `seedance_2_0` + `mode:'fast'`("Seedance 2.0 Fast", 동일 720p·4초·`generate_audio:false`. `fast` 모드는 1080p 미지원이나 이 플레이북은 720p라 무관) → ③ Fast도 안 될 때만 백업2(최종) `seedance_2_0` + `mode:'std'`("Seedance 2.0 standard", 모델 기본 모드와 동일 파라미터). Mini 미확인을 std로 대신 채우지 않는다 — 미확인이면 곧장 ②로 간다.
...
| 영상 모델 | `seedance_2_0`(기본, "Mini" 프리셋)·`kling3_0_turbo`·`kling3_0`·`veo3_1` … (`model list --video`) | `seedance_2_0`(기본)·`kling3_0_turbo` | (`video_models_list`) |
...
# 영상화: 확정 이미지(job ID 또는 경로)를 첫 프레임으로 (기본 모델 = seedance_2_0 "Mini", 720p·4초·무음)
higgsfield generate create seedance_2_0 --prompt "<규칙0[B] No-BGM + VIDEO_PROMPT_FORMAT 7필드(700자 이내)>" --image <이미지_jobID_또는_경로> --duration 4 --resolution 720p --generate_audio false --wait
# 백업1 (Mini 카탈로그에 안 잡힐 때) — 동일 명령에 --mode fast 추가
# 백업2 (Fast도 실패할 때, 최종) — 동일 명령에 --mode std 추가(모델 기본값과 동일)
