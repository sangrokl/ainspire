...
| **영상화** | **Higgsfield CLI / MCP(`minimax_hailuo` + `--model minimax-2.3`, 기본) · Magnific MCP** | CLI `higgsfield generate create minimax_hailuo --model minimax-2.3 … --image` · MCP `generate_video` · Magnific `video_generate` (백업 모델: `seedance_2_0`) |
...
4. **기본 모델(크레딧 절약, 2026-06-22부터):** Higgsfield 연결 시 **개별 컷 이미지 = `nano_banana_2`(Pro) · 2K · Unlimited ON**, **영상 = `minimax_hailuo`(`--model minimax-2.3`) · 768(=768p) · 6초**를 기본값으로 쓴다(백업: 영상 `seedance_2_0`). `minimax_hailuo`는 `--aspect_ratio`/`--start-image` 플래그가 없다 — 입력은 `--image`(첫 프레임), 비율은 입력 이미지를 따른다(`higgsfield-ai/cli` MODELS.md 기준, 2026-06-22 확인). Unlimited 토글의 정확한 파라미터명은 호출 전 `models_explore(action='get', model_id='minimax_hailuo')`로 확정.
...
| 영상 모델 | `minimax_hailuo`(기본, `--model minimax-2.3`)·`seedance_2_0`·`kling3_0`·`veo3_1` … (`model list --video`) | `minimax_hailuo`(기본)·`seedance_2_0` | (`video_models_list`) |
...
# 영상화: 확정 이미지(job ID 또는 경로)를 첫 프레임으로 (기본 모델 = minimax_hailuo --model minimax-2.3, 768·6초 — 크레딧 절약. 백업: seedance_2_0)
higgsfield generate create minimax_hailuo --model minimax-2.3 --prompt "<규칙0[B] No-BGM + VIDEO_PROMPT_FORMAT 7필드(700자 이내)>" --image <이미지_jobID_또는_경로> --duration 6 --resolution 768 --wait
...
[제작 — 스토리보드 확정 후]
→ G12 영상화(Minimax Hailuo 2.3, 기본) → G13 BGM(Suno 프롬프트→사용자 생성) → G14 내레이션 VO(ElevenLabs MCP 기본, 백업 Higgsfield TTS)
